// Browser Console Test Script for CRUD Operations
...
const runCRUDTests = async () => {
  ...
  try {
    const { entities } = await import('/src/api/supabaseEntities.js');
    ...
    for (const [entityName, data] of Object.entries(testData)) {
      if (entities[entityName]) {
        ...
        log(`Entity ${entityName} not found in entities`, 'error');
        testResults.failed.push({ entity: entityName, error: 'Entity not found' });
      }
    }
  } catch (error) {
    log(`Failed to load entities: ${error.message}`, 'error');
    testResults.failed.push({ entity: 'Entities', error: error.message });
  }
