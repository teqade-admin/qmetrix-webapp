...
import {
  ...
  ChevronLeft, ChevronRight, Send, AlertCircle, Users, Lock, Search
} from "lucide-react";
...
/**
 * TimesheetView
 * @param {"self"|"team"} scope        - "self" shows the signed-in user's own timesheets (editable);
 *                                        "team" shows the team's timesheets (read + approve only).
 ...
 * @param {boolean} canApprove         - whether the user may approve submitted entries.
 */
export default function TimesheetView({ scope = "self", currentEmployeeName = "", teamNames = [], canApprove = false }) {
  ...
  const timesheets = useMemo(() => {
    if (isSelf) return allTimesheets.filter(t => t.employee_name === currentEmployeeName);
    return allTimesheets.filter(t => teamNames.includes(t.employee_name) && t.status !== "draft");
  }, [allTimesheets, isSelf, currentEmployeeName, teamNames]);

  useEffect(() => {
    if (isSelf && !editing && dialogOpen && !form.employee_name) {
      setForm(f => ({ ...f, employee_name: currentEmployeeName }));
    }
  }, [currentEmployeeName, dialogOpen, editing, form.employee_name, isSelf]);
  ...
  // Weekly target scales with headcount: 40h per person. Self = 1, Team = number of members.
  const employeeCount = isSelf ? 1 : teamNames.length;
  const weekTarget = 40 * employeeCount;
  ...
  // All tab filter. Team has no drafts, and defaults to the approval queue (submitted).
  const statusFilters = isSelf
    ? ["all", "draft", "submitted", "approved", "rejected"]
    : ["submitted", "approved", "rejected"];
  const [allFilter, setAllFilter] = useState(isSelf ? "all" : "submitted");
  ...
  const allPager = usePagination(filteredAll, 10);
  ...
  const handleSave = e => {
    ...
    if (editing && editing.status === "approved") {
      setFormError("This entry has been approved and can no longer be edited.");
      return;
    }
  ...
  return (
    <div className="space-y-5">
      {/* Header row — only "self" can log new hours */}
      {isSelf && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">Log and submit your billable and non-billable hours.</p>
      ...
      {/* Stats */}
      <div className={`grid grid-cols-2 gap-3 ${isSelf ? "lg:grid-cols-4" : "lg:grid-cols-5"}`}>
        {!isSelf && (
          <StatCard title="Total Employees" value={employeeCount} icon={Users} color="primary" />
        )}
        <StatCard title="This Week" value={`${weekTotal.toFixed(1)}h`} subtitle={`of ${weekTarget}h target`} icon={Clock} color="primary" />
      ...
      <Tabs value={isSelf ? tab : "all"} onValueChange={setTab}>
        {isSelf && (
          <TabsList>
          ...
          <Card>
            <CardContent className="pt-4 pb-2">
              ...
              <div className="flex items-center justify-between mt-1">
                ...
                <Progress value={Math.min((weekTotal / weekTarget) * 100, 100)} className="w-32 h-1.5" />
              </div>
            </CardContent>
          </Card>
                      ...
                      {dayEntries.map(ts => (
                        <div key={ts.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-muted/20">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            {ts.status === "approved" ? (
                              <Lock className="h-3 w-3 text-muted-foreground" title="Approved — locked" />
                            ) : (
                              <>
                                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => openEdit(ts)} title="Edit">
                                  <Pencil className="h-3 w-3" />
                                </Button>
                                <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => setDeleteId(ts.id)} title="Delete">
        ...
        <TabsContent value="all" className="mt-4">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
            ...
            <div className="relative">
              ...
              <Input placeholder="Search…" value={allSearch} onChange={e => setAllSearch(e.target.value)} className="pl-9 w-44 h-8" />
            </div>
          </div>
          <Card>
                        ...
                        <div className="flex gap-1">
                          {canApprove && ts.status === "submitted" && (
                            <>
                              ...
                              <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" title="Reject"
                          ...
                          {isSelf && (
                            ts.status === "approved" ? (
                              ...
                              <>
                                <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openEdit(ts)} title="Edit">
                                  <Pencil className="h-3.5 w-3.5" />
                                </Button>
                                <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => setDeleteId(ts.id)} title="Delete">
